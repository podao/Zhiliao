...
class FollowButton : IHook {
    ...
    private lateinit var bottomReactionView: Class<*>
    private lateinit var followPeopleButton: Class<*>
    private lateinit var zHAuthorInfoView: Class<*>
    ...
    @Throws(Throwable::class)
    override fun hook() {
        XposedBridge.hookAllMethods(bottomReactionView, "setData", object : XC_MethodHook() {
            @Throws(Throwable::class)
        ...
        XposedBridge.hookAllMethods(zHAuthorInfoView, "setData", object : XC_MethodHook() {
            @Throws(Throwable::class)
            override fun afterHookedMethod(param: MethodHookParam) {
                if (Helper.prefs.getBoolean("switch_mainswitch", false)
                    && Helper.prefs.getBoolean("switch_subscribe", false)
